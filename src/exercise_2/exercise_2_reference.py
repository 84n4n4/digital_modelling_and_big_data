...
def only_tall_male_students(df):
    return df[(df['Sex'] == 'Male') & (df['Height'] > 170)]
...
def year_started(df):
    now = 2024
    df['YearStarted'] = now - df['YearsStudying']
    return df
...
def group_means(df):
    return df[['Height', 'Age', 'YearsStudying', 'Sex']].groupby(['Sex']).mean()
...
def concat_dataframes(df_a, df_b):
    return pandas.concat([df_a, df_b])
...
def main():
    ...
    print('Output of sort_by_height()\n')
